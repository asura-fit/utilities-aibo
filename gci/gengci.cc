/*
  $Log: gengci.cc,v $
  Revision 1.3  2002/06/19 12:30:39  ken
...
LoadThresholds(const char* filename)
{
  ...
  for(int i=0;i<fNumOfColor;i++) {
    ...
    if (fThreshs[i].S_min > fThreshs[i].S_max) {
      ...
      fprintf(stderr, "COLOR %d will be rejected.\n", i);
    }
...
main(int argc, char* argv[])
{
  ...
  printf("gengci version 2002/3/28 v.04\n");
  if (argc <= 2)
	  printf("gengci <color.lst> <MAUTO.GCI>\n");
